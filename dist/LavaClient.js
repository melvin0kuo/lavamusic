  ...
  static updateStatus(client2, guildId) {
    ...
    if (user && client2.env.GUILD_ID && guildId === client2.env.GUILD_ID) {
      ...
      user.setPresence({
        activities: [
          {
            name: player?.queue?.current ? `\u{1F3B6} | ${player.queue?.current.info.title}` : client2.env.BOT_ACTIVITY,
            type: player?.queue?.current ? import_discord.ActivityType.Listening : client2.env.BOT_ACTIVITY_TYPE
          }
        ],
