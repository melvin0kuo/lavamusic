...
module.exports = __toCommonJS(GuildCreate_exports);
...
// src/events/client/GuildCreate.ts
var GuildCreate = class extends Event {
  static {
    __name(this, "GuildCreate");
  }
  ...
  async run(guild) {
    ...
    const embed = new import_discord6.EmbedBuilder().setColor(this.client.config.color.green).setAuthor({
      name: guild.name,
      iconURL: guild.iconURL({
        extension: "jpeg"
      })
    }).setDescription(`**${guild.name}** has been added to my guilds!`).setThumbnail(guild.iconURL({
      extension: "jpeg"
    })).addFields({
      name: "Owner",
      ...
      name: "Created At",
      value: `<t:${Math.floor(guild.createdTimestamp / 1e3)}:F>`,
      inline: true
    }, {
      name: "Joined At",
      value: `<t:${Math.floor(guild.joinedTimestamp / 1e3)}:F>`,
      inline: true
    }, {
      name: "ID",
      value: guild.id,
