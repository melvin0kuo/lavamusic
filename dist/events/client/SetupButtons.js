    ...
    if (message) {
      ...
      switch (interaction.customId) {
        ...
        case "SKIP_BUT": {
          ...
          const newEmbed = this.client.embed().setAuthor({
            name: T(locale, "event.setupButton.now_playing"),
            iconURL: this.client.config.icons[newTrack?.sourceName || ""] || this.client.user?.displayAvatarURL({
              extension: "png"
            })
          }).setColor(this.client.color.main).setDescription(`[${newTrack?.title}](${newTrack?.uri}) - ${newTrack?.isStream ? T(locale, "event.setupButton.live") : this.client.utils.formatTime(newTrack?.duration)} - ${T(locale, "event.setupButton.requested_by", {
            requester: (player.queue.current?.requester).id
          })}`).setImage(newTrack?.artworkUrl || this.client.user?.displayAvatarURL({
            extension: "png"
          }));
          if (message) {
            await message.edit({
