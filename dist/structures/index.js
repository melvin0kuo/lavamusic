...
var Utils = class {
        ...
        activities: [
          {
            ...
            type: player?.queue?.current ? import_discord3.ActivityType.Listening : client.env.BOT_ACTIVITY_TYPE
          }
        ],
  ...
  static async paginate(client, ctx, embed) {
    ...
    const getButton = /* @__PURE__ */ __name((page2) => {
      ...
      const first = new import_discord3.ButtonBuilder().setCustomId("first").setEmoji(client.emoji.page.first).setStyle(import_discord3.ButtonStyle.Primary).setDisabled(firstEmbed);
      const back = new import_discord3.ButtonBuilder().setCustomId("back").setEmoji(client.emoji.page.back).setStyle(import_discord3.ButtonStyle.Primary).setDisabled(firstEmbed);
      const next = new import_discord3.ButtonBuilder().setCustomId("next").setEmoji(client.emoji.page.next).setStyle(import_discord3.ButtonStyle.Primary).setDisabled(lastEmbed);
      const last = new import_discord3.ButtonBuilder().setCustomId("last").setEmoji(client.emoji.page.last).setStyle(import_discord3.ButtonStyle.Primary).setDisabled(lastEmbed);
      const stop = new import_discord3.ButtonBuilder().setCustomId("stop").setEmoji(client.emoji.page.cancel).setStyle(import_discord3.ButtonStyle.Danger);
      const row = new import_discord3.ActionRowBuilder().addComponents(first, back, stop, next, last);
      return {
        embeds: [
          pageEmbed
        ],
    ...
    const author = ctx instanceof import_discord3.CommandInteraction ? ctx.user : ctx.author;
...
// src/structures/Lavamusic.ts
var Lavamusic = class extends import_discord4.Client {
  static {
  ...
  db = new ServerData();
  cooldown = new import_discord4.Collection();
  ...
  manager;
  embed() {
    return new import_discord4.EmbedBuilder();
  }
  async start(token) {
    initI18n();
    if (env.TOPGG) {
    ...
    this.on(import_discord4.Events.InteractionCreate, async (interaction) => {
  ...
  async loadCommands() {
    const commandsPath = import_node_fs2.default.readdirSync(import_node_path3.default.join(process.cwd(), "dist", "commands"));
    for (const dir of commandsPath) {
      const commandFiles = import_node_fs2.default.readdirSync(import_node_path3.default.join(process.cwd(), "dist", "commands", dir)).filter((file) => file.endsWith(".js"));
      for (const file of commandFiles) {
        const cmdModule = require(import_node_path3.default.join(process.cwd(), "dist", "commands", dir, file));
        ...
        if (command.slashCommand) {
          const data = {
            name: command.name,
            description: T(import_discord5.Locale.EnglishUS, command.description.content),
            type: import_discord4.ApplicationCommandType.ChatInput,
            options: command.options || [],
            default_member_permissions: Array.isArray(command.permissions.user) && command.permissions.user.length > 0 ? import_discord4.PermissionsBitField.resolve(command.permissions.user).toString() : null,
          ...
          if (command.options.length > 0) {
            command.options.map((option) => {
              ...
              option.description = T(import_discord5.Locale.EnglishUS, option.description);
            });
            data.options?.map((option) => {
              if ("options" in option && option.options.length > 0) {
                option.options?.map((subOption) => {
                  ...
                  subOption.description = T(import_discord5.Locale.EnglishUS, subOption.description);
                });
              }
            });
          }
  ...
  async deployCommands(guildId) {
    const route = guildId ? import_discord4.Routes.applicationGuildCommands(this.user?.id ?? "", guildId) : import_discord4.Routes.applicationCommands(this.user?.id ?? "");
    try {
      const rest = new import_discord4.REST({
        version: "10"
      }).setToken(env.TOKEN ?? "");
  ...
  async loadEvents() {
    const eventsPath = import_node_fs2.default.readdirSync(import_node_path3.default.join(process.cwd(), "dist", "events"));
    for (const dir of eventsPath) {
      const eventFiles = import_node_fs2.default.readdirSync(import_node_path3.default.join(process.cwd(), "dist", "events", dir)).filter((file) => file.endsWith(".js"));
      for (const file of eventFiles) {
        const eventModule = require(import_node_path3.default.join(process.cwd(), "dist", "events", dir, file));
