...
// src/commands/config/247.ts
var _247 = class extends Command {
  static {
    __name(this, "_247");
  }
  constructor(client) {
    ...
    try {
      ...
      if (!member.voice.channel) {
        return await ctx.sendMessage({
          embeds: [
            embed.setDescription(ctx.locale("cmd.247.errors.not_in_voice")).setColor(client.color.red)
          ]
        });
      }
      if (data) {
        ...
        return await ctx.sendMessage({
          embeds: [
            embed.setDescription(ctx.locale("cmd.247.messages.disabled")).setColor(client.color.red)
          ]
        });
      }
      ...
      return await ctx.sendMessage({
        embeds: [
          embed.setDescription(ctx.locale("cmd.247.messages.enabled")).setColor(this.client.color.main)
        ]
      });
    } catch (error) {
