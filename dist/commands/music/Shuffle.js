...
// src/commands/music/Shuffle.ts
var Shuffle = class extends Command {
  static {
    __name(this, "Shuffle");
  }
  constructor(client) {
  ...
  async run(client, ctx) {
    ...
    if (fairPlay) {
      return await ctx.sendMessage({
        embeds: [
          embed.setColor(this.client.color.red).setDescription(ctx.locale("cmd.shuffle.errors.fairplay"))
        ]
      });
    }
    ...
    return await ctx.sendMessage({
      embeds: [
        embed.setColor(this.client.color.main).setDescription(ctx.locale("cmd.shuffle.messages.shuffled"))
      ]
    });
  }
};
