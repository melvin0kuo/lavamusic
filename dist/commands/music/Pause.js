...
// src/commands/music/Pause.ts
var Pause = class extends Command {
  static {
    __name(this, "Pause");
  }
  constructor(client) {
  ...
  async run(client, ctx) {
    ...
    if (player?.paused) {
      return await ctx.sendMessage({
        embeds: [
          embed.setColor(this.client.color.red).setDescription(ctx.locale("player.errors.already_paused"))
        ]
      });
    }
    player?.pause();
    return await ctx.sendMessage({
      embeds: [
        embed.setColor(this.client.color.main).setDescription(ctx.locale("cmd.pause.successfully_paused"))
      ]
    });
  }
};
