...
// src/commands/music/Replay.ts
var Replay = class extends Command {
  static {
    __name(this, "Replay");
  }
  constructor(client) {
  ...
  async run(client, ctx) {
    ...
    if (!player.queue.current?.info.isSeekable) {
      return await ctx.sendMessage({
        embeds: [
          embed.setColor(this.client.color.red).setDescription(ctx.locale("cmd.replay.errors.not_seekable"))
        ]
      });
    }
    player.seek(0);
    return await ctx.sendMessage({
      embeds: [
        embed.setColor(this.client.color.main).setDescription(ctx.locale("cmd.replay.messages.replaying"))
      ]
    });
  }
};
