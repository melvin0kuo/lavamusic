...
// src/commands/music/Remove.ts
var Remove = class extends Command {
  static {
    __name(this, "Remove");
  }
  constructor(client) {
  ...
  async run(client, ctx, args) {
    ...
    if (player.queue.tracks.length === 0) return await ctx.sendMessage({
      embeds: [
        embed.setColor(this.client.color.red).setDescription(ctx.locale("cmd.remove.errors.no_songs"))
      ]
    });
    const songNumber = Number(args[0]);
    if (Number.isNaN(songNumber) || songNumber <= 0 || songNumber > player.queue.tracks.length) return await ctx.sendMessage({
      embeds: [
        embed.setColor(this.client.color.red).setDescription(ctx.locale("cmd.remove.errors.invalid_number"))
      ]
    });
