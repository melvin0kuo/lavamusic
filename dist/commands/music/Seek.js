...
// src/commands/music/Seek.ts
var Seek = class extends Command {
  static {
    __name(this, "Seek");
  }
  constructor(client) {
  ...
  async run(client, ctx, args) {
    ...
    if (!duration) {
      return await ctx.sendMessage({
        embeds: [
          embed.setColor(this.client.color.red).setDescription(ctx.locale("cmd.seek.errors.invalid_format"))
        ]
      });
    }
    if (!current?.isSeekable || current.isStream) {
      return await ctx.sendMessage({
        embeds: [
          embed.setColor(this.client.color.red).setDescription(ctx.locale("cmd.seek.errors.not_seekable"))
        ]
      });
    }
