...
// src/commands/music/Queue.ts
var Queue = class extends Command {
  static {
    __name(this, "Queue");
  }
  constructor(client) {
  ...
  async run(client, ctx) {
      ...
      return await ctx.sendMessage({
        embeds: [
          embed.setColor(this.client.color.main).setDescription(ctx.locale("cmd.queue.now_playing", {
            ...
            duration: player.queue.current.info.isStream ? ctx.locale("cmd.queue.live") : client.utils.formatTime(player.queue.current.info.duration)
          }))
        ]
      });
    }
    ...
    const pages = chunks.map((chunk, index) => {
      return this.client.embed().setColor(this.client.color.main).setAuthor({
