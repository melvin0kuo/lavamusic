...
module.exports = __toCommonJS(Leave_exports);
...
// src/commands/music/Leave.ts
var Leave = class extends Command {
  static {
    __name(this, "Leave");
  }
  constructor(client) {
  ...
  async run(client, ctx) {
    ...
    if (player) {
      ...
      return await ctx.sendMessage({
        embeds: [
          embed.setColor(this.client.color.main).setDescription(ctx.locale("cmd.leave.left", {
            channelId
          }))
        ]
      });
    ...
    return await ctx.sendMessage({
      embeds: [
        embed.setColor(this.client.color.red).setDescription(ctx.locale("cmd.leave.not_in_channel"))
      ]
    });
  }
};
