...
// src/commands/music/Nowplaying.ts
var Nowplaying = class extends Command {
  static {
    __name(this, "Nowplaying");
  }
  constructor(client) {
    super(client, {
      name: "nowplaying",
  ...
  async run(client, ctx) {
    ...
    const embed = this.client.embed().setColor(this.client.color.main).setAuthor({
      name: ctx.locale("cmd.nowplaying.now_playing"),
      iconURL: ctx.guild?.iconURL({})
    }).setThumbnail(track.info.artworkUrl).setDescription(ctx.locale("cmd.nowplaying.track_info", {
