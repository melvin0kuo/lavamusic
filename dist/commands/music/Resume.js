...
// src/commands/music/Resume.ts
var Resume = class extends Command {
  static {
    __name(this, "Resume");
  }
  constructor(client) {
  ...
  async run(client, ctx) {
    ...
    if (!player.paused) {
      return await ctx.sendMessage({
        embeds: [
          embed.setColor(this.client.color.red).setDescription(ctx.locale("cmd.resume.errors.not_paused"))
        ]
      });
    }
    player.resume();
    return await ctx.sendMessage({
      embeds: [
        embed.setColor(this.client.color.main).setDescription(ctx.locale("cmd.resume.messages.resumed"))
      ]
    });
  }
};
