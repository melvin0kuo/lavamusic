...
// src/commands/music/PlayLocal.ts
var PlayLocal_exports = {};
__export(PlayLocal_exports, {
  default: () => PlayLocal
});
module.exports = __toCommonJS(PlayLocal_exports);
...
// src/commands/music/PlayLocal.ts
var PlayLocal = class extends Command {
  static {
    __name(this, "PlayLocal");
  }
  constructor(client) {
    super(client, {
      name: "playlocal",
      ...
      options: [
        {
          ...
          type: import_discord6.ApplicationCommandOptionType.Attachment,
          required: true
        }
      ]
    });
  ...
  async run(client, ctx) {
    const attachment = ctx.isInteraction ? ctx.interaction.options.get("file")?.attachment : ctx.message?.attachments.first();
    if (!attachment) {
      return ctx.sendMessage({
        embeds: [
          this.client.embed().setColor(this.client.color.red).setDescription(ctx.locale("cmd.playlocal.errors.empty_query"))
        ]
      });
    }
    ...
    if (!audioExtensions.includes(`.${extension}`)) {
      return ctx.sendMessage({
        embeds: [
          this.client.embed().setColor(this.client.color.red).setDescription(ctx.locale("cmd.playlocal.errors.invalid_format"))
        ]
      });
    }
    ...
    const response = await player.search({
      ...
      source: "local"
    }, ctx.author).catch(() => null);
    if (!response || !response.tracks?.length) {
      return ctx.editMessage({
        content: " ",
        embeds: [
          this.client.embed().setColor(this.client.color.red).setDescription(ctx.locale("cmd.playlocal.errors.no_results"))
        ]
      });
    }
    ...
    await ctx.editMessage({
      content: "",
      embeds: [
        this.client.embed().setColor(this.client.color.main).setDescription(ctx.locale("cmd.playlocal.added_to_queue", {
