...
var Help = class extends Command {
  ...
  async run(client, ctx, args) {
    ...
    if (args[0]) {
      ...
      const helpEmbed2 = embed.setColor(client.color.main).setTitle(`${ctx.locale("cmd.help.title")} - ${command.name}`).setDescription(ctx.locale("cmd.help.help_cmd", {
    ...
    const helpEmbed = embed.setColor(client.color.main).setTitle(ctx.locale("cmd.help.title")).setDescription(ctx.locale("cmd.help.content", {
      bot: client.user?.username,
      prefix: guild.prefix
    })).setFooter({
      text: ctx.locale("cmd.help.footer", {
        prefix: guild.prefix
      })
    }).addFields(...fields);
    return await ctx.sendMessage({
