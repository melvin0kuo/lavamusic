...
var Botinfo_exports = {};
__export(Botinfo_exports, {
  default: () => Botinfo
});
module.exports = __toCommonJS(Botinfo_exports);
var import_node_os = __toESM(require("os"));
var import_discord6 = require("discord.js");
var import_node_system_stats = require("node-system-stats");
...
// src/commands/info/Botinfo.ts
var Botinfo = class extends Command {
  static {
    __name(this, "Botinfo");
  }
  constructor(client) {
  ...
  async run(client, ctx) {
    const osInfo = `${import_node_os.default.type()} ${import_node_os.default.release()}`;
    const osUptime = client.utils.formatTime(import_node_os.default.uptime());
    const osHostname = import_node_os.default.hostname();
    const cpuInfo = `${import_node_os.default.arch()} (${import_node_os.default.cpus().length} cores)`;
    const cpuUsed = (await (0, import_node_system_stats.usagePercent)({
      coreIndex: 0,
      sampleMs: 2e3
    })).percent;
    const memTotal = (0, import_node_system_stats.showTotalMemory)(true);
    const memUsed = (process.memoryUsage().rss / 1024 ** 2).toFixed(2);
    const nodeVersion = process.version;
    const discordJsVersion = import_discord6.version;
    const commands = client.commands.size;
    const promises = [
      client.shard?.broadcastEval((client2) => client2.guilds.cache.size),
      client.shard?.broadcastEval((client2) => client2.guilds.cache.reduce((acc, guild) => acc + guild.memberCount, 0)),
      client.shard?.broadcastEval((client2) => client2.channels.cache.size)
    ];
