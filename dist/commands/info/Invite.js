...
// src/commands/info/Invite.ts
var Invite = class extends Command {
  static {
    __name(this, "Invite");
  }
  constructor(client) {
  ...
  async run(client, ctx) {
    ...
    const row = new import_discord6.ActionRowBuilder().addComponents(new import_discord6.ButtonBuilder().setLabel(ctx.locale("buttons.invite")).setStyle(import_discord6.ButtonStyle.Link).setURL(`https://discord.com/api/oauth2/authorize?client_id=${client.env.CLIENT_ID}&permissions=8&scope=bot%20applications.commands`), new import_discord6.ButtonBuilder().setLabel(ctx.locale("buttons.support")).setStyle(import_discord6.ButtonStyle.Link).setURL("https://discord.gg/STXurwnZD5"));
