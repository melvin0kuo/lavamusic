...
var CreateInvite_exports = {};
__export(CreateInvite_exports, {
  default: () => CreateInvite
});
module.exports = __toCommonJS(CreateInvite_exports);
...
// src/commands/dev/CreateInvite.ts
var CreateInvite = class extends Command {
  static {
    __name(this, "CreateInvite");
  }
  constructor(client) {
    super(client, {
      name: "createinvite",
      description: {
        ...
        examples: [
          "createinvite 0000000000000000000"
        ],
  ...
  async run(client, ctx, args) {
    ...
    const textChannel = guild.channels.cache.find((c) => c.type === import_discord6.ChannelType.GuildText && c.permissionsFor(guild.members.me)?.has(import_discord6.PermissionFlagsBits.CreateInstantInvite | import_discord6.PermissionFlagsBits.SendMessages | import_discord6.PermissionFlagsBits.ViewChannel));
    if (!textChannel) {
      return await ctx.sendMessage({
        embeds: [
          this.client.embed().setColor(this.client.color.red).setDescription("No suitable channel found")
        ]
      });
    }
