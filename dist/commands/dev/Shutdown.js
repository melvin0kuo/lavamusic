...
// src/commands/dev/Shutdown.ts
var Shutdown_exports = {};
__export(Shutdown_exports, {
  default: () => Shutdown
});
module.exports = __toCommonJS(Shutdown_exports);
...
// src/commands/dev/Shutdown.ts
var Shutdown = class extends Command {
  static {
    __name(this, "Shutdown");
  }
  constructor(client) {
    super(client, {
      name: "shutdown",
  ...
  async run(client, ctx) {
    ...
    const button = new import_discord6.ButtonBuilder().setStyle(import_discord6.ButtonStyle.Danger).setLabel("Confirm Shutdown").setCustomId("confirm-shutdown");
    const row = new import_discord6.ActionRowBuilder().addComponents(button);
    const shutdownEmbed = embed.setColor(this.client.color.red).setDescription(`**Are you sure you want to shutdown the bot **\`${client.user?.username}\`?`).setTimestamp();
    ...
    const filter = /* @__PURE__ */ __name((i) => i.customId === "confirm-shutdown" && i.user.id === ctx.author?.id, "filter");
