...
// src/commands/dev/Restart.ts
var Restart = class extends Command {
  static {
    __name(this, "Restart");
  }
  constructor(client) {
  ...
  async run(client, ctx) {
    ...
    const button = new import_discord6.ButtonBuilder().setStyle(import_discord6.ButtonStyle.Danger).setLabel("Confirm Restart").setCustomId("confirm-restart");
    const row = new import_discord6.ActionRowBuilder().addComponents(button);
    const restartEmbed = embed.setColor(this.client.color.red).setDescription(`**Are you sure you want to restart **\`${client.user?.username}\`?`).setTimestamp();
    ...
    const filter = /* @__PURE__ */ __name((i) => i.customId === "confirm-restart" && i.user.id === ctx.author?.id, "filter");
    ...
    collector.on("collect", async (i) => {
      ...
      try {
        ...
        (0, import_node_child_process.exec)("npm run start", (error, stdout, stderr) => {
          if (error) {
