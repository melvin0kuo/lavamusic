...
var GuildList_exports = {};
__export(GuildList_exports, {
  default: () => GuildList
});
module.exports = __toCommonJS(GuildList_exports);
...
// src/commands/dev/GuildList.ts
var GuildList = class extends Command {
  static {
    __name(this, "GuildList");
  }
  constructor(client) {
    super(client, {
      name: "guildlist",
  ...
  async run(client, ctx) {
    const guilds = await client.shard?.broadcastEval((c) => c.guilds.cache.map((guild) => ({
      name: guild.name,
      id: guild.id
    })));
    ...
    const pages = chunks.map((chunk, index) => {
      return this.client.embed().setColor(this.client.color.main).setDescription(chunk.join("\n")).setFooter({
