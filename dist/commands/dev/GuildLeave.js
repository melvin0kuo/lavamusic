...
// src/commands/dev/GuildLeave.ts
var GuildLeave_exports = {};
__export(GuildLeave_exports, {
  default: () => GuildLeave
});
module.exports = __toCommonJS(GuildLeave_exports);
...
// src/commands/dev/GuildLeave.ts
var GuildLeave = class extends Command {
  static {
    __name(this, "GuildLeave");
  }
  constructor(client) {
    super(client, {
      name: "guildleave",
  ...
  async run(client, ctx, args) {
    ...
    const guild = await client.shard?.broadcastEval((c, { guildId: guildId2 }) => {
      const guild2 = c.guilds.cache.get(guildId2);
      return guild2 ? {
        id: guild2.id,
        name: guild2.name
      } : null;
    ...
    try {
      await client.shard?.broadcastEval(async (c, { guildId: guildId2 }) => {
        const guild2 = c.guilds.cache.get(guildId2);
        if (guild2) {
          await guild2.leave();
        }
    ...
    if (logChannelId) {
      ...
      if (logChannel && logChannel.type === import_discord6.ChannelType.GuildText) {
