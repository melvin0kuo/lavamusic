...
// src/commands/playlist/RemoveSong.ts
var RemoveSong_exports = {};
__export(RemoveSong_exports, {
  default: () => RemoveSong
});
module.exports = __toCommonJS(RemoveSong_exports);
...
// src/commands/playlist/RemoveSong.ts
var RemoveSong = class extends Command {
  static {
    __name(this, "RemoveSong");
  }
  constructor(client) {
    super(client, {
      name: "removesong",
  ...
  async run(client, ctx, args) {
    ...
    if (!playlist) {
      const errorMessage = this.client.embed().setDescription(ctx.locale("cmd.removesong.messages.provide_playlist")).setColor(this.client.color.red);
    ...
    if (!song) {
      const errorMessage = this.client.embed().setDescription(ctx.locale("cmd.removesong.messages.provide_song")).setColor(this.client.color.red);
      return await ctx.sendMessage({
        embeds: [
          errorMessage
        ]
    ...
    if (!playlistData) {
      const playlistNotFoundError = this.client.embed().setDescription(ctx.locale("cmd.removesong.messages.playlist_not_exist")).setColor(this.client.color.red);
      return await ctx.sendMessage({
        embeds: [
    ...
    if (!res || res.loadType !== "track") {
      const noSongsFoundError = this.client.embed().setDescription(ctx.locale("cmd.removesong.messages.song_not_found")).setColor(this.client.color.red);
      return await ctx.sendMessage({
        embeds: [
          noSongsFoundError
        ]
    ...
    try {
      ...
      const successMessage = this.client.embed().setDescription(ctx.locale("cmd.removesong.messages.song_removed", {
      ...
      console.error(error);
      const genericError = this.client.embed().setDescription(ctx.locale("cmd.removesong.messages.error_occurred")).setColor(this.client.color.red);
      return await ctx.sendMessage({
        embeds: [
          genericError
        ]
